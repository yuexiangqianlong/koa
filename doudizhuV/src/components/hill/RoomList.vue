		...
		<el-table :data="roomList" row-key="id">
		    ...
		    <el-table-column
		      ...
		      <template scope="scope">
		        <el-button
		          type="text"
		          size="small" @click.native.prevent='go(scope.$index,true)'>
		          进入
		        </el-button>
		      </template>
		    </el-table-column>
...
<script>
import {newRoom} from "../../vuex/actions/HallAction"
import {getRoomList} from '../../vuex/actions/HallAction'
import {go} from '../../vuex/actions/RoomAction'
export default {
	data(){
  	return {
  		roomList:[]
 ...
 newRoom:function(){
     let formObj='roompwd='+newRoomForm.roompwd.value;
  	...
  	go:function(id){
  		//console.log(this);
  		//alert(this.$data.roomList[id].room);
  		
  		let room= this.$data.roomList[id].room;
  		  // alert(room);
  		go(room,this);
  	}
  }
